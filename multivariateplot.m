function ret = multivariateplot(opt)
if opt == 1
load('params1.mat')
params1(:,11) = -params1(:,11);
params1(:,12) = -params1(:,12);
mu = mean(log(params1(:,1:12)));
Sigma = cov(log(params1(:,1:12)));
elseif opt == 2
    load('params2.mat')
params2(:,11) = -params2(:,11);
params2(:,12) = -params2(:,12);
mu = mean(log(params2(:,1:12)));
Sigma = cov(log(params2(:,1:12)));
elseif opt == 3
    
    load('params3.mat')
params3(:,11) = -params3(:,11);
params3(:,12) = -params3(:,12);
mu = mean(log(params3(:,1:12)));
Sigma = cov(log(params3(:,1:12)));
end
sample = exp(mvnrnd(mu,Sigma,10000));
sample(:,11) = -sample(:,11);
sample(:,12) = -sample(:,12);
...
prim_starts = unique(sort(sample(:,11)));
sec_starts = unique(sort(sample(:,12)));
...
for j = 1:length(sample(:,1))
    k_prim = find(sample(j,11) == starts);
    k_sec = find(sample(j,12) == starts);
    t_prim = [starts(k_prim:end)',starts(end)+.05:.05:20];
    t_sec = [starts(k_sec:end)',starts(end)+.05:.05:20];
    
    sol_prim = paramfunPrim(sample(j,1:8),t_prim);
    sol_sec = paramfunSec(sample(j,1:10),t_sec);
    ...
    id = find(abs(sol_prim(:,1)) >= .1);
    if length(id) > 0
    Vstart_prim(j) = t_prim(id(1))-t_prim(1);
    Vend_prim(j) = t_prim(id(end))-t_prim(1);
    Vdur_prim(j) = Vend_prim(j)-Vstart_prim(j);
    ...
    id = find(abs(sol_sec(:,1)) >= .1);
    if length(id) > 0
    Vstart_sec(j) = t_sec(id(1))-t_sec(1);
    Vend_sec(j) = t_sec(id(end))-t_sec(1);
    Vdur_sec(j) = Vend_sec(j)-Vstart_sec(j);
    elseif length(id) == 0
...
virus_prim = sort(virus_prim);
cross_prim = sort(cross_prim);
spec_prim = sort(spec_prim);
virus_sec = sort(virus_sec);
cross_sec = sort(cross_sec);
spec_sec = sort(spec_sec);
remove = 250;
...
plot(pat1Tprim,pat1Vprim,'-','Color',red,'Marker','d')
end
...
set(h,'facealpha',.4)
plot(t_all,median(cross_prim),'Color','black','linewidth',2,'LineStyle','-','Marker','None')
...
h = fill(x, inBetween,right_color,'LineStyle','none','Marker','none');
set(h,'facealpha',.2)
plot(t_all,median(spec_prim),'Color','black','linewidth',2,'LineStyle','-','Marker','None')
...
plot(pat1Tsec,pat1Vsec,'-','Color',red,'Marker','d')
end
% for r1p = 1:length(indDPatprim)
%    
% if r1p < length(indDPatprim)
...
h = fill(x, inBetween,right_color,'LineStyle','none','Marker','none');
set(h,'facealpha',.4)
plot(t_all,median(cross_sec),'Color','black','linewidth',2,'LineStyle','-','Marker','None')
% plot(t_all,spec_prim(remove+1,:),'Color',blue,'LineStyle','-','Marker','None')
% plot(t_all,spec_prim(end-remove,:),'Color',blue,'LineStyle','-','Marker','None')
inBetween = [spec_sec(remove+1,:),fliplr(spec_sec(end-remove,:))];
h = fill(x, inBetween,right_color,'LineStyle','none','Marker','none');
set(h,'facealpha',.2)
plot(t_all,median(spec_sec),'Color','black','linewidth',2,'LineStyle','-','Marker','None')
xlabel('Time since symptom onset (days)')
ylabel('IgG [$\log_{10}(\mu g /mL)$]')
ylim([0 9])
if opt == 1
